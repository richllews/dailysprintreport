...
def parse_issues(issue_data):
    ...
    for issue in issue_data['issues']:
        ...
        #ßprint(f"Processing issue {issue['key']} of type {issue_type}")  # Debug print
        changelog = get_issue_changelog(issue['id'])

         # Tracking changes in active status for timeline regardless of issue type
        ...
        if in_progress_date:
         ...
         #   print(f"Excluding issue {issue['key']} from cycle time calculation")  # Debug print 
        #continue

         # Cycle time calculation, excluding specified issue types
...
def plot_data_and_save_html(issues, active_state_dates):
    ...
    fig_active.update_layout(title='Work in Progress', xaxis_title='Date', yaxis_title='Number of Tickets')
    ...
    with open('Daily Report' + sprint_id + '.html', 'w') as f:
        f.write(html)
        print('Daily Report' + sprint_id + '.html' + ' created')
